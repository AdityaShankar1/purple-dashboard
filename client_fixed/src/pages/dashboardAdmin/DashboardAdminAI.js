...
export default function DashboardAdminAI() {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-140px)] overflow-hidden bg-[var(--bg-primary)]">
            {/* Header */}
            <div className="bg-[var(--card-bg)] border-b border-[var(--card-border)] p-5 flex justify-between items-center">
                <div className="flex items-center space-x-4">
                    <div className="bg-[#a855f7] p-2.5 rounded-xl shadow-lg">
                        <Bot size={28} className="text-white" />
                    </div>
                    <div>
                        <h2 className="font-extrabold text-xl tracking-tight text-slate-900 dark:text-[#f6e6f7]">Security Assistant</h2>
                        <div className="flex items-center text-xs font-medium text-slate-500 dark:text-slate-400">
                            ...
                            {connStatus === 'failed' && <AlertCircle size={12} className="text-rose-500 mr-2" />}

                            {connStatus === 'checking' && "Checking connectivity..."}
                            {connStatus === 'wazuh' && "Powered by Ollama, Always Verify Results"}
                            {connStatus === 'dashboard' && "Powered by Ollama, Always Verify Results"}
                            {connStatus === 'mock' && "Powered by Ollama, Always Verify Results"}
                ...
                <div className="flex items-center space-x-2">
                    <button
                        ...
                        disabled={loading}
                        className="flex items-center space-x-2 px-4 py-2 bg-[var(--bg-secondary)] hover:bg-[var(--accent-purple)]/10 disabled:opacity-50 rounded-xl transition-all text-sm font-bold text-[var(--accent-purple)] border border-[var(--card-border)]"
                        title="Quick Summary"
                    >
                        <Sparkles size={18} />
                        <span>Summarize</span>
                    ...
                    <button
                        ...
                        className="p-2.5 hover:bg-rose-500/10 rounded-xl transition-all text-slate-400 hover:text-rose-500"
                        title="Clear History"
                    >
                        <Trash2 size={22} />
                    </button>
            ...
            <div className="p-5 bg-[var(--card-bg)]/60 backdrop-blur-md border-t border-[var(--card-border)]">
                ...
                <p className="text-[11px] text-center mt-3 text-gray-400 font-medium">AI Insights are powered by Purple SOC Engine (Ollama). Always verify critical alerts manually.</p>
            </div>
        </div>
    );
}
